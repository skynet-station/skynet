...
const generateResponse = (response, scenario) => {
	...
	if (event === "start" && depth === 0 && response === "gas") {
		...
		return { video: "./gas_1.webm", image: "./tesla.png", choices: ["예", "아니요"] };
	}
	...
	if (event === "start" && depth === 0 && response === "start") {
		...
		return {
			...
			choices: ["기름 넣으러 왔어요", "출차 부탁드립니다", "오늘 날씨 알려주세요", "오늘 열리는 행사가 있나요?"],
		};
	}
	...
	if (event === "gas" && depth === 1 && response === "yes") {
		...
		ScenarioContext.render();
		return { video: "./gas_2_yes.webm", image: "./oilstation.png", choices: ["가득 넣어주세요", "절반만 넣어주세요"] };
	}
	...
	if (event === "gas" && depth === 2) {
		...
		ScenarioContext.render();
		return { video: "./gas_3.webm", image: "./oilstation.png", choies: ["감사합니다"] };
	}
	...
	if (event === "leave" && depth === 2) {
		...
		ScenarioContext.render();
		return { video: "./leave_3.webm", image: "./oilstation.png", choices: ["고마워요~"] };
	}
